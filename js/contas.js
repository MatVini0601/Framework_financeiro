...
function lancarContas(){
  ...
  if(verificar(Categoria)){
    if(!localStorage.getItem('Contas')){
      ...
      let tipo = document.getElementById('txttipo').value
                var CadastroContas = {
                    id: id++,titulo,categoria:dados,tipo,Cliente
                }
  ...
  function listarContas(){
    ...
    if(ContasLancadas == null || ContasLancadas == "[]"){
      let tabela = document.getElementById('tabelaCol')
      tabela.innerHTML = "Não há nada para ser listado. Tente cadastrar uma conta"
    }else{
